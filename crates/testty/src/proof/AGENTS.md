...
- [`backend.rs`](backend.rs) - `ProofBackend` trait and `ProofFormat` enum for swappable
  proof output.
- [`report.rs`](report.rs) - `ProofReport` collector, `ProofCapture`, `AssertionResult`,
  and annotated text output.
...
- [`html.rs`](html.rs) - Self-contained HTML report backend with embedded images and
  diff summaries.

...
pub struct Session {
    ...
    /// `summary.session` field and, once the session reaches `Done`, formatted
    /// with `# Summary` and `# Commit` sections using the canonical session
    /// commit message. This text is also reused as review-assist context.

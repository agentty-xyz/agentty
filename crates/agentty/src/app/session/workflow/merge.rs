    ...
    async fn restore_review_status(context: &MergeStartRestoreContext<'_>) {
        match context.status_transition.apply(Status::Review).await {
            ...
            Ok(false) => warn!(
                ...
                "skipped restoring review status because the persisted status was already current"
            ),
            Err(error) => warn!(
                session_id = context.session_id,
                error = %error,
        ...
        match result {
            ...
            Err(error) => {
                ...
                match status_transition.apply(Status::Review).await {
                    ...
                    Ok(false) => warn!(
                        session_id = id,
                        "skipped restoring review status after merge error because the persisted \

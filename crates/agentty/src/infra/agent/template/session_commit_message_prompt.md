...
Before writing the message, inspect repository commit-message guidance from relevant agent instruction files (`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`) and relevant skills under shared or agent-specific skill directories (for example `skills/`, `.claude/skills/`, `.gemini/skills/`, `.codex/skills/`, `.agents/skills/`). Check the skill files that appear relevant to commit-message conventions when those paths exist.
Use the most specific applicable repository guidance you find unless explicit user instructions in the diff request a different format.
...
- Base the title and body on the diff content and the existing session commit message, while applying any commit-format requirements discovered in the checked agent files and skills.
- Do not invent changes, rationale, or formatting rules that are not supported by the diff or the discovered repository guidance.

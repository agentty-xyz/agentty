...
- Keep workflow policy in `app`; infra implements capabilities rather than deciding
  application flow.
- Access agent lifecycle services through `ag-worker`, clipboard reads from
  `ag-clipboard`, Git operations from `ag-git`, and persistence from `ag-store` instead
  of recreating them here.

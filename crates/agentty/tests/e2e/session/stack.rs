...
async fn seed_review_ready_parent_with_review_child(
    env: &BuilderEnv,
...
async fn seed_pending_post_merge_restack_child(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
...
async fn seed_failing_pending_post_merge_restack_child(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
    let child_worktree = env.agentty_root.join("wt").join("stack-re");
...
async fn seed_dirty_fork_source_session(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
...
#[tokio::test]
async fn stacked_session_creation() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("stacked_session_creation")
        .with_git()
        .setup(|env| Box::pin(async move { seed_four_level_review_stack(env).await }))
        .run(
            |scenario| {
                scenario
...
#[tokio::test]
async fn append_session_to_stack() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("append_session_to_stack")
        .with_git()
        .setup(|env| Box::pin(async move { seed_appendable_review_sessions(env).await }))
        .zola(
...
#[tokio::test]
async fn stacked_parent_merge_remains_available_with_review_child() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("stacked_parent_merge_remains_available_with_review_child")
        .with_git()
        .setup(|env| Box::pin(async move { seed_review_ready_parent_with_review_child(env).await }))
        .run(
            |scenario| {
                scenario
                    .compose(&common::wait_for_agentty_startup())
...
#[tokio::test]
async fn stacked_pending_post_merge_restack_recovers_on_startup() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("stacked_pending_post_merge_restack_recovers_on_startup")
        .with_git()
        .setup(|env| Box::pin(async move { seed_pending_post_merge_restack_child(env).await }))
        .run(
            |scenario| {
                scenario
                    .compose(&common::wait_for_agentty_startup())
...
#[tokio::test]
async fn stacked_pending_post_merge_restack_failure_is_visible() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("stacked_pending_post_merge_restack_failure_is_visible")
        .with_git()
        .setup(|env| {
            Box::pin(async move { seed_failing_pending_post_merge_restack_child(env).await })
        })
        .run(
            |scenario| {
                scenario
...
#[tokio::test]
async fn stacked_session_start_waits_for_parent_review() -> E2eResult {
    // Arrange, Act, Assert
...
/// session view.
#[tokio::test]
async fn session_fork_confirmation_creates_session_from_review_session() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("session_fork_confirmation")
        .with_git()
        .with_terminal_size(120, 24)
        .setup(|env| Box::pin(async move { seed_dirty_fork_source_session(env).await }))
        .run(
            |scenario| {
                scenario
                    .compose(&common::wait_for_agentty_startup())
...
#[tokio::test]
async fn stacked_child_hides_session_fork_shortcut() -> E2eResult {
    // Arrange, Act, Assert

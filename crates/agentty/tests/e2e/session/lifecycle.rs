...
async fn seed_session_with_reasoning_level(
    env: &BuilderEnv,
...
async fn seed_sessions_with_matching_update_times(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
    common::seed_session(
        env,
...
/// draft flow.
async fn seed_done_session_for_continuation(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
...
/// draft flow.
async fn seed_canceled_session_for_continuation(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
    common::seed_session(
        env,
...
#[tokio::test]
async fn session_list_empty_state() -> E2eResult {
    // Arrange, Act, Assert
...
#[tokio::test]
async fn session_list_model_reasoning_level() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("session_list_model_reasoning_level")
        .with_git()
        .setup(|env| Box::pin(async move { seed_session_with_reasoning_level(env).await }))
        .run(
            |scenario| {
                scenario
                    .compose(&common::wait_for_agentty_startup())
...
#[tokio::test]
async fn session_list_matching_update_times_use_creation_order() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("session_list_creation_order")
        .with_git()
        .setup(|env| Box::pin(async move { seed_sessions_with_matching_update_times(env).await }))
        .run(
            |scenario| {
                scenario
                    .compose(&common::wait_for_agentty_startup())
...
#[tokio::test]
async fn existing_session_keeps_persisted_reasoning_label() -> E2eResult {
    // Arrange, Act, Assert
...
#[tokio::test]
async fn session_list_selected_row_remains_readable_under_dark_horizon() -> E2eResult {
    // Arrange, Act, Assert
...
#[tokio::test]
async fn session_stop_turn_returns_to_review() -> E2eResult {
    // Arrange, Act, Assert
...
async fn seed_delayed_session_creation(env: &BuilderEnv) -> E2eResult {
...
#[tokio::test]
async fn session_creation_keeps_navigation_responsive() -> E2eResult {
    // Arrange
    FeatureTest::new("session_creation_responsive")
        .with_git()
        .setup(|env| Box::pin(async move { seed_delayed_session_creation(env).await }))
        .run(
            |scenario| {
                // Act
                scenario
...
#[tokio::test]
async fn session_creation_runs_early_prompt_once() -> E2eResult {
    // Arrange
    FeatureTest::new("session_creation_early_prompt")
        .with_git()
        .setup(|env| {
            Box::pin(async move {
                seed_delayed_session_creation(env).await?;
                seed_claude_structured_output_project(env).await
            })
        })
        .run(
            |scenario| {
...
#[tokio::test]
async fn session_creation_recovers_unreleased_first_prompt() -> E2eResult {
    // Arrange
...
#[tokio::test]
async fn session_creation_retries_rejected_handoff() -> E2eResult {
    // Arrange
                    ...
                    connection
                        .execute(
                            "CREATE TABLE reject_first_handoff (pending INTEGER); INSERT INTO \
                             ...
                             END; CREATE TRIGGER allow_retry AFTER UPDATE ON session_preparation \
                             WHEN NEW.state = 'failed' BEGIN DELETE FROM reject_first_handoff; \
                             END;",
                        )
                        .await?;
                    connection.close().await
...
#[tokio::test]
async fn session_creation_opens_prompt_mode() -> E2eResult {
    // Arrange
    FeatureTest::new("session_creation")
        .with_git()
        .setup(|env| Box::pin(async move { seed_sessions_tab(env).await }))
        .zola(
            "Session creation",
...
#[tokio::test]
async fn draft_session_creation_opens_staging_mode() -> E2eResult {
    // Arrange
...
#[tokio::test]
async fn session_open_and_return_to_list() -> E2eResult {
    // Arrange, Act, Assert
        ...
        .run(
            ...
            |frame, report| {
                Box::pin(async move {
                    ...
                    let view_full =
                        Region::full(session_view_frame.cols(), session_view_frame.rows());
...
/// draft composer.
#[tokio::test]
async fn terminal_session_continue_opens_seeded_prompt() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("terminal_session_continue")
        .with_git()
        .setup(|env| Box::pin(async move { seed_done_session_for_continuation(env).await }))
        .zola(
...
#[tokio::test]
async fn canceled_session_continue_opens_seeded_prompt() -> E2eResult {
    // Arrange, Act, Assert
    FeatureTest::new("canceled_session_continue")
        .with_git()
        .setup(|env| Box::pin(async move { seed_canceled_session_for_continuation(env).await }))
        .zola(
...
#[tokio::test]
async fn session_list_jk_navigation() -> E2eResult {
    // Arrange, Act, Assert
                ...
                Box::pin(async move {
                    ...
                    assert!(
                        down_navigation_text.contains("alpha")
                            || down_navigation_text.contains("beta"),

...
pub(crate) async fn seed_active_project_setting(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
    (async {
        ...
        let project_id = database
            ...
            .await?;
        crate::test_support::persist_active_project_id_for_test(&database, project_id).await?;
...
/// be created.
pub(crate) async fn seed_second_project(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
    seed_additional_project(env, "zeta-project").await
}
...
/// Returns an error if the project repository or persisted metadata cannot
/// be created.
pub(crate) async fn seed_mru_first_second_project(
    env: &BuilderEnv,
) -> Result<(), Box<dyn std::error::Error>> {
    seed_additional_project(env, "alpha-project").await
}

/// Seed one additional never-opened Git project with the provided directory
/// and display label.
async fn seed_additional_project(
    env: &BuilderEnv,
    project_name: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    let temp_root = env
    ...
    (async {
        ...
        database
            ...
            .await?;
        crate::test_support::persist_active_tab_for_test(&database, agentty::app::Tab::Sessions)
            .await?;
...
pub(crate) async fn acquire_e2e_test_lock() -> MutexGuard<'static, ()> {
    static E2E_TEST_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

    E2E_TEST_LOCK.get_or_init(|| Mutex::new(())).lock().await
}
...
type SetupFuture<'a> = Pin<Box<dyn Future<Output = Result<(), Box<dyn std::error::Error>>> + 'a>>;
type AssertFuture<'a> = Pin<Box<dyn Future<Output = ()> + 'a>>;
type FeatureSetupHook = Box<dyn for<'a> FnOnce(&'a BuilderEnv) -> SetupFuture<'a>>;
...
impl FeatureTest {
    ...
    pub(crate) fn setup(
        mut self,
        setup: impl for<'a> FnOnce(&'a BuilderEnv) -> SetupFuture<'a> + 'static,
    ) -> Self {
    ...
    pub(crate) async fn run(
        ...
        assert: impl for<'a> FnOnce(&'a TerminalFrame, &'a ProofReport) -> AssertFuture<'a>,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let _test_guard = acquire_e2e_test_lock().await;
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            demo.run(&scenario, builder, &vhs_binary_path, &env_pairs)
        ...
        assert(&result.frame, &result.report).await;

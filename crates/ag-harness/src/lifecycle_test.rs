...
use std::sync::{Arc, Barrier, Condvar, Mutex, PoisonError, mpsc};
use std::thread;
use std::time::{Duration, Instant};

use super::{
    LifecycleEmitter, LifecycleEvent, LifecycleEventKind, LifecycleId, LifecycleObserver,
    LifecycleObserverSet, LifecycleOperationGuard, ModelResponseType, ToolErrorType, TurnErrorType,
};
use crate::model::{CompletionMetadata, ModelErrorType};
...
fn starts_tool_execution_timer_after_started_observer() {
    ...
    let emitter = LifecycleEmitter::new(move |event: LifecycleEvent| {
        if matches!(event.kind(), LifecycleEventKind::ToolStarted { .. }) {
            *recorded_time.lock().unwrap_or_else(PoisonError::into_inner) = Some(Instant::now());
        }
    });

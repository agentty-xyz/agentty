...
async fn resolution_peels_once_and_validates_the_returned_commit() {
    ...
    runner
        ...
        .in_sequence(&mut sequence)
        .withf(|_, arguments| {
            arguments == ["--no-replace-objects", "cat-file", "-t", COMPARISON_OID]
        })
    ...
    runner
        ...
        .returning(|_, _| Ok(output(COMPARISON_OID)));
...
async fn validation_requires_commit_type_and_canonicalizes_full_oids() {
    ...
    for oid in ["a".repeat(40), "b".repeat(64)] {
        ...
        runner
            ...
            .times(1)
            .in_sequence(&mut sequence)
            .return_once(move |_, _| Ok(output(oid)));
    }
...
#[tokio::test]
async fn sha256_validation_rejects_a_resolvable_forty_character_prefix() {
    // Arrange
    let fixture = ComparisonRepository::with_object_format("sha256").await;
    let prefix = &fixture.base[..40];

    // Act
    let resolved = ComparisonBase::resolve(&fixture.repository, prefix)
        .await
        .expect("host revision resolves to a full SHA-256 commit");
    let validated = ComparisonBase::validate(&fixture.repository, &fixture.base.to_uppercase())
        .await
        .expect("full SHA-256 commit");
    let abbreviated = ComparisonBase::validate(&fixture.repository, prefix).await;

    // Assert
    assert_eq!(fixture.base.len(), 64);
    assert_eq!(resolved.oid(), fixture.base);
    assert_eq!(resolved, validated);
    assert!(matches!(abbreviated, Err(ComparisonBaseError::InvalidOid)));
}

#[tokio::test]
async fn validation_and_resolution_use_original_objects_despite_replacements() {
    // Arrange
    ...
    tokio::fs::write(replacements.join(&fixture.base), &fixture.blob)
        .await
        .expect("commit replaced by blob");
    tokio::fs::write(replacements.join(&fixture.blob), &fixture.next)
        .await
        .expect("blob replaced by commit");

    // Act
    let resolved = ComparisonBase::resolve(&fixture.repository, "v1")
        .await
        .expect("original tagged commit");
    let validated = ComparisonBase::validate(&fixture.repository, &fixture.base)
        .await
        .expect("original commit");
    let invalid = ComparisonBase::validate(&fixture.repository, &fixture.blob).await;

    // Assert
    assert_eq!(resolved.oid(), fixture.base);
    assert_eq!(resolved, validated);
    assert!(matches!(invalid, Err(ComparisonBaseError::NotCommit)));
}

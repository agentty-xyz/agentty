...
async fn reopening_preserves_comparison_continuation_across_key_order_and_budget_changes() {
    ...
    let turns: Vec<_> = [(schemas[0], 2), (schemas[1], 2), (schemas[1], 3)]
        ...
        .collect();
    let requests = Arc::new(Mutex::new(Vec::<ModelRequest>::new()));
    let database_path = fixture.directory.path().join("history.db");
    let new_harness = || {
        let recorded = Arc::clone(&requests);
        let mut model = model();
    ...
    // Act
    for (index, options) in turns.into_iter().enumerate() {
        let harness = new_harness();
        let mut session = if index == 0 {
            harness
                .session("session", options.schema().clone())
                .create()
                .await
                .expect("new session")
        } else {
...
async fn comparison_scope_mismatch_fails_before_calling_the_model() {
    ...
    assert_eq!(
        error.error_type(),
        TurnErrorType::ComparisonRepositoryMismatch
    );
    assert!(matches!(
        durable,
        Err(SessionError::Turn(TurnError::ComparisonRepositoryMismatch))
    ));
    let failures = events
        .lock()
        .expect("events lock")
        ...
        .filter_map(|event| match event.kind() {
            LifecycleEventKind::TurnFailed { error_type, .. } => Some(*error_type),
            _ => None,
        })
        .collect::<Vec<_>>();
    assert_eq!(failures, [TurnErrorType::ComparisonRepositoryMismatch; 2]);
    assert_eq!(failures[0].as_str(), "comparison_repository_mismatch");
}

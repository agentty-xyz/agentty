...
use crate::{
    ...
    NewSession, OutputSchema, SessionError, SessionStore, SqliteStore, ToolPolicy, TurnLimits,
    TurnOptions, TurnOwner,
};
...
async fn both_stores_block_unknown_commands_and_reconcile_only_the_original_owner() {
    ...
    for store in stores {
        ...
        store
            ...
            .expect("session");
        let first = store
            .begin_turn(Arc::clone(&store), "commands", "first", &options, 0)
            .await
            .expect("first");
        ...
        let blocked = store
            .begin_turn(Arc::clone(&store), "commands", "second", &options, 0)
            .await;
...
async fn unresolved_cleanup_remains_blocking_after_outcome_recording() {
    ...
    for store in stores {
        ...
        let blocked = store
            .begin_turn(Arc::clone(&store), "commands", "next", &options, 0)
            .await;
...
async fn duplicate_requests_classify_before_pending_command_admission() {
    ...
    for store in stores {
        ...
        let crate::HostTurnAcquisition::Acquired(acquired) = store
            .begin_request(Arc::clone(&store), "commands", "run", &options, &request, 0)
            .await
            .expect("acquire")
        else {

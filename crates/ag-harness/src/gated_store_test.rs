//! Session store decorator that pauses lease and terminal operations for
...
pub(crate) struct GatedStore {
    ...
    pub(crate) write_intents: AtomicUsize,
    pub(crate) write_outcomes: AtomicUsize,
}

impl GatedStore {
    pub(crate) fn new(database: Database, pause_at: PauseAt) -> Self {
        Self {
            database,
            entered: Notify::new(),
    ...
    pub(crate) async fn fixture(pause_at: PauseAt) -> (Arc<Self>, AcquiredTurn) {

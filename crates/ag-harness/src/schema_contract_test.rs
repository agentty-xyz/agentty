...
use super::{
    ContentSizeWriter, DIAGNOSTIC_LIMIT_CHARS, OutputSchema, OutputSchemaError,
    OutputValidationError, RESPONSE_CONTENT_LIMIT_BYTES, SCHEMA_LIMIT_BYTES, bounded_diagnostic,
};
...
fn rejects_oversized_structured_value() {
    // Arrange
    let schema = OutputSchema::new(json!({ "type": "string" })).expect("schema should be valid");

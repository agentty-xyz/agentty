...
static int child(const char *pid_path, const char *release, const char *result,
    ...
    /* Publish atomically: observers wait for the path, then read it. */
    if (snprintf(temporary, sizeof temporary, "%s.tmp", result) >= sizeof temporary) return 10;
    FILE *marker = fopen(temporary, "w");
    if (!marker) return 13;
    fputs("confined", marker);
    fclose(marker);
    if (rename(temporary, result)) return 14;
    return 0;
}

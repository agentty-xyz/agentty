...
pub use ag_worker::{OperationRepository, SessionOperationRow};
...
#[async_trait]
impl OperationRepository<DbError> for SqliteOperationRepository {
...
",
    ...
    async fn heartbeat(&self, operation_id: &str) -> Result<(), DbError> {
        let now = self.timestamp_source.now_timestamp_seconds();
        sqlx::query!(
            "UPDATE session_operation SET heartbeat_at = ? WHERE id = ? AND status = 'running'",
            now,
            operation_id
        )
        .execute(&self.pool)

...
1. **Version Bump**
   ...
   - Verify `Cargo.lock` is updated with `prek run cargo-check --all-files`.

1. **Verification**

   - Run the final validation hooks from the `prek` hook catalog in `.pre-commit-config.yaml`:
     - `prek run --all-files`
...
1. **Documentation Review**
   ...
   - Run `prek run zola-check --all-files --hook-stage manual` to verify no broken internal links.

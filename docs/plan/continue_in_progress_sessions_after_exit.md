...
- [ ] **Persist detached turn payloads end to end.** Add the `session_operation` migration in `crates/agentty/migrations/` for immutable detached-runner fields (`prompt`, `model`, `turn_mode`, `resume_output`, `runner_pid`, reusing `heartbeat_at`), then add the detached-operation model and DB accessors in `crates/agentty/src/infra/db.rs` so one queued turn can be loaded from storage without relying on TUI memory.
- [ ] **Launch detached execution through one boundary.** Add the `RunnerSpawner` trait in `crates/agentty/src/infra/runner.rs` and the `agentty run-turn <operation-id>` subcommand in `crates/agentty/src/main.rs` so persisted turns start through one explicit detached-runner entrypoint instead of ad hoc process spawning.
- [ ] **Move turn ownership and final persistence into the detached runner.** Shift CLI child processes and app-server runtimes into the detached execution path, then persist final transcript, terminal status, and durable `session.output` state back through `crates/agentty/src/app/session/workflow/worker.rs` or a new `crates/agentty/src/app/session/workflow/turn_runner.rs` module before the runner exits.
- [ ] **Keep reopen behavior safe before full reconciliation lands.** Stop blanket startup recovery in `crates/agentty/src/app/core.rs` from force-failing detached-enabled unfinished work so reopening during an active detached turn may still be stale, but it never corrupts the persisted operation before the later reconciliation slice lands.
- [ ] **Add the rollback switch for staged rollout.** Support `AGENTTY_DETACH=0` so detached execution can be disabled cleanly and the app can fall back to the current in-process worker model while the later detached slices stabilize.

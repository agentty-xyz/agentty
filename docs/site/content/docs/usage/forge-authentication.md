...
<a id="usage-forge-authentication"></a>
Agentty uses plain Git for branch publishing and then uses the forge CLI for
pull-request or merge-request actions.
...
<a id="usage-forge-authentication-github"></a>
Use this setup when the repository remote is on GitHub.

1. Run `gh auth login`.
1. Choose the target host such as `github.com`.
1. Pick the Git protocol Agentty should use for branch publishing:
   `https` or `ssh`.
1. If you picked `https`, run `gh auth setup-git` after login so Git can use
   `gh` as a credential helper for branch pushes.
...
<a id="usage-forge-authentication-gitlab"></a>
...
1. If you picked `https`, also configure Git itself for HTTPS pushes:
   either a credential helper or a personal access token that Git can use when
   `git push` prompts for credentials.

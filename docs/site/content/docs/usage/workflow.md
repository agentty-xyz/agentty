...
repositories) in a dashboard view: equal-width activity heatmap and Agentty info panels
(version, short description) on top, project table with project names, session counts,
and last-opened dates below. Agentty skips stale entries whose project directories no
longer exist or no longer contain git metadata. | | **Sessions** | List, create, and
manage agent sessions. When a project is active, this tab appears as

...
repositories) in a dashboard view: activity heatmap and Agentty info (version, short
description) on top, project table with project names, session counts, and last-opened
dates below. Agentty skips stale entries whose project directories no longer exist or no

...
- Claude, Gemini, and Codex session turns use strict parsing and fail closed when output
  does not match the protocol schema. Antigravity session turns use the same strict
  parse and one protocol-repair retry first, then preserve non-empty plain text as
  `answer` when `agy --print` ignores both schema prompts.
...
- Wrapped stream chunks that end in one valid protocol JSON object are reduced to that
  ...
  succeeds.
- Gemini ACP final turn assembly prefers the completed `session/prompt` payload when it
  contains valid protocol JSON and the earlier streamed chunk accumulation does not, so
  strict validation sees the authoritative structured response.
